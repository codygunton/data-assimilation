...
def RK_matrix(N):
    ...
    for i in range(N):
        ...
        np.put(e, i, 1)
        standard_basis += [e]
    k1 = f(standard_basis, 0)
    k2 = f(standard_basis + h*k1/2, 0 + h/2)
    k3 = f(standard_basis + h*k2/2, 0 + h/2)
    k4 = f(standard_basis + h*k3, 0 + h)
    result = standard_basis + (h/6)*(k1 + 2*k2 + 2*k3 + k4)
    return result.T
...
h = 0.005                         # time step
x0 = np.load("starting-data.npy")
RK_times = np.arange(h, 200.0, h)  # number of time steps
t0 = time.time()
cProfile.run('states = L96(x0, RK_times)')
t1 = time.time()
print("time:", t1-t0)
