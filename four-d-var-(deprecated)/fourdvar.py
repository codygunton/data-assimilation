...
def fun_r_RK(x, mu_0, y, sqrtB_inv, sqrtR_inv):  # f = (1/2)r^Tr
    r1 = sqrtB_inv @ (x - mu_0)
    r2 = sqrtR_inv @ (H @ M_RK(x) - y)
    r = np.concatenate((r1, r2))
    return r


def fun_J_RK(x, sqrtB_inv, sqrtR_inv):  # the jacobian of fun_r_RK
    J1 = sqrtB_inv
    J2 = sqrtR_inv @ H @ DM_RK(x)
    J = np.concatenate((J1, J2))
    return J


def fourdvar_fun_RK(mu_0, B_0, R, y):
    # notation: moving from time 0 to time next
    # y should be obs[n] if at time 0
    ...
    # run gauss-newton
    x_0_opt = lsqs(lambda x:
                   fun_r_RK(x, mu_0, y, sqrtB_0_inv, sqrtR_inv),
                   mu_0,
                   jac=lambda x:
                   fun_J_RK(x, sqrtB_0_inv, sqrtR_inv)).x

    mu_next = M_RK(mu_0)  # bad name
    mu_next_opt = M_RK(x_0_opt)

    M_lin = DM_RK(x_0_opt)
    J = fun_J_RK(x_0_opt, sqrtB_0_inv, sqrtR_inv)
    B_0_post = np.linalg.inv(2*J.T @ J)
