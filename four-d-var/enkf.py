...
n = (len(true_states)-1)//(len(obs)+1)    # number of iters between obsns
...
def main(alpha, r, obs, log_name):
    ...
    for y in obs:
        ...
        infl_ens_f = np.array([mu_f + np.sqrt(1+alpha)*(e - mu_f)
                               for e in ens_f])
        mu, infl_P_f = ens_mean_cov(infl_ens_f)
...
(ensembles,
 ...
 P_as,
 RMSEs,
 spreads) = main(0.26, 5.5, obs, 'whatever')
